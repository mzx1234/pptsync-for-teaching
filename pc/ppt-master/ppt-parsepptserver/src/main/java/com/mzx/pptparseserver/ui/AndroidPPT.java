...
//                _currentPage = globalApplication.getLen() - 1;
        ...
        _broadcastIP.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                BroadcastIpServer broadcastIpServer = (BroadcastIpServer) Main.getBean("broadcastIpServerImpl");
                broadcastIpServer.boadcastIp(_ipString);
            }
        });
    ...
    protected void showPPT(String path) {
        ...
        byte[] bytes = null;
        try {
            bytes = pptOption.load(path, _fileName);
        } catch (Exception ex) {
