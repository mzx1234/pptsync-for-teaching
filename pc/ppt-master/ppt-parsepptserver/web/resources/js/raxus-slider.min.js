...
function raxusSliderGlobal(that, direction) {
    ...
    if (totalW == pixel && direction == "-")if (loop)return false; else {
        dataShow = 0;
        e.attr("data-show", "0")
    } else if (dataShow == 0 && direction == "+")if (loop)return false; else {
        dataShow = totalSize;
    ...
    move(rel, action);
    if (dataShow == totalSize)jQuery(that).parents(".raxus-slider[data-loop]").find(".arrow-r").css("opacity", "0.5"); else if (dataShow == 0)jQuery(that).parents(".raxus-slider[data-loop]").find(".arrow-l").css("opacity", "0.5"); else jQuery(that).parents(".raxus-slider[data-loop]").find(".arrow-r, .arrow-l").css("opacity", "1");
    if (e.find(".mini-images").length) {
        ...
        if (miniSize - 1 >= miniShow) {
            if (miniSelected == miniImgsLi.slice(-1).index() || miniSelected == miniImgsLi.slice(-2).index())thumbPixel = -((miniSize - miniShow) * miniSlide) + margin, number = miniImgsLi.last().index() - miniShow; else if (miniSelected == miniSize && miniShow <= miniSize)thumbPixel = -miniSelected * miniSlide + miniSlide * miniShow, number = miniSelected - miniShow + 1; else if (miniSelected == 0)thumbPixel = 0, number = miniSelected; else if (miniSelected >= firstDisplay + miniShow)thumbPixel =
                -miniSelected * miniSlide + miniSlide * (miniShow - 1), number = miniSelected - miniShow + 1; else if (miniSelected <= firstDisplay)thumbPixel = -(miniSelected - 1) * miniSlide, number = miniSelected - 1; else thumbPixel = miniImgs.attr("data-thumbwidth");
...
function thumbNoSlide() {
    jQuery(rS + "[data-thumbnail]").each(function () {
        if (jQuery(this).find(".mini-images li").size() * jQuery(this).find(".mini-images li").outerWidth(true) < jQuery(this).outerWidth())jQuery(this).find(".mini-images").addClass("no-slide")
    })
}
...
jQuery(function () {
    ...
    function autoPlay(time, that) {
        ...
        function startPlay() {
            if (!that.find(".close-video").length)that.find(".arrow-r").addClass("autoPlayButton");
    ...
    function touching(a) {
        dirAndThumb(a);
        if (a.className == "slider-relative")slideWhere = direction ? parseInt(jQuery(a).position().top) : parseInt(jQuery(a).position().left); else if (a.className == "mini-images")slideWhere =
    ...
    function touchable(that, b, e, event) {
        ...
        if (that.className == "slider-relative no-transition") {
            ...
            if (-(startX - b) > 0)var tWay = thumb ? "0, 0" : "0, 0"; else if (-(startX - b) < finish)var tWay = thumb ? "0," + finish + "px" : finish + "px, 0"; else {
    ...
    jQuery(document).on("mouseup touchend", rS + " .slider-relative", function () {
        ...
        if (-startX < -100)raxusSliderGlobal(thatR, "-"); else if (-startX > 100)raxusSliderGlobal(thatL, "+");
        rePositioning()
    });
    jQuery(document).on("mouseup touchend", rS + " .mini-images", function (event) {
        ...
        if (endDir > -10)move(jQuery(this), "0, 0"); else if (endDir <=
            finish) {
    ...
    jQuery(".raxus-slider").keydown(function (event) {
        if (event.which == 39)raxusSliderGlobal("[data-keypress=true]:focus .arrow-r", "-"); else if (event.which == 37)raxusSliderGlobal("[data-keypress=true]:focus .arrow-l", "+")
    });
    jQuery(document).on("click touchstart",
        ".fullscreen", function () {
            ...
            if (!document.fullscreenElement && !document.mozFullScreenElement && !document.webkitFullscreenElement && !document.msFullscreenElement)
                if (elem.requestFullscreen)elem.requestFullscreen(); else if (elem.msRequestFullscreen)elem.msRequestFullscreen(); else if (elem.mozRequestFullScreen)elem.mozRequestFullScreen(); else {
                if (elem.webkitRequestFullscreen)elem.webkitRequestFullscreen()
            } else if (document.exitFullscreen)document.exitFullscreen();
            else if (document.msExitFullscreen)document.msExitFullscreen(); else if (document.mozCancelFullScreen)document.mozCancelFullScreen(); else if (document.webkitExitFullscreen)document.webkitExitFullscreen()
        });
    document.addEventListener("fullscreenchange", function () {
        if (document.fullscreenElement)rePositioning(); else rePositioning()
    }, false);
    document.addEventListener("msfullscreenchange", function () {
        if (document.msFullscreenElement)rePositioning(); else rePositioning()
    }, false);
    document.addEventListener("mozfullscreenchange",
        function () {
            if (document.mozFullScreen)rePositioning(); else rePositioning()
        }, false);
    document.addEventListener("webkitfullscreenchange", function () {
        if (document.webkitIsFullScreen)rePositioning(); else rePositioning()
    }, false);
